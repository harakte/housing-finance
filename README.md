...
- 2018 년도 금융지원 금액을 예측하는 방법
  ...
  - Linear regression class 결정
    - @Component로 했을 경우 
      - 예측에 사용할 값을 constructor를 통해 미리 초기화를 하게 되면 다른 은행의 예측이 들어왔을 때 초기화 값이 변경되게 됨.
    - static function으로 했을 경우
      - 매번 값을 받아 계산하여야 함.
    - 객체 생성하여 사용할 경우
      - constructor를 사용하여 사용 값을 미리 계산해서 저장해 둘 수 있음
      - 하나의 처리 안에서 같은 은행에 대해서는 달을 바꿔가며 값을 받아오기 용이
